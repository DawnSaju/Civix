...
export async function getGeminiResponse(
  ...
  try {
    ...
    // Extract grounding metadata with more robust checks
    ...
    if (groundingMetadata?.groundingChunks) {
      ...
      // Fallback: If we have an entry point but no chunks, at least provide the Google link
    ...
    try {
      ...
      // Merge sources from grounding metadata with any the model explicitly listed
    ...
    // Explicitly handle Rate Limiting (429)
    if (error.status === 429 || error.message?.includes("429") || error.message?.includes("quota")) {
      return JSON.stringify({
        thought: ["Quota exceeded"],
        answer: "You've been rate limited, Please try again later.",
        sources: []
      });
    }
